...
check(html.includes('src/main.js?v=50.00'), 'App lädt Main mit Phase-42-Cache-Stand.');
check(main.includes('WorkspaceComponent.js?v=50.00'), 'Main lädt Workspace cache-sicher.');
...
check(version.includes("APP_RELEASE = '50.00'"), 'Release ist Phase 42.00.');
check(version.includes("APP_VERSION = '2.5.0'"), 'App-Version ist 1.19.0.');
check(packageJson.version === '2.5.0', 'Paketversion ist 1.19.0.');
check(packageJson.scripts['test:phase42']?.includes('run-phase42-table-import-tests.js'), 'Phase-42-Testskript ist registriert.');
check(packageJson.scripts.test.startsWith('node tests/run-phase50-formpart-workflow-tests.js'), 'Gesamttest startet mit Phase 42.');
check(landing.includes('Release 2.5 · Phase 50.00'), 'Index-Hauptseite nennt den aktuellen Stand.');
