...
check(workspace.includes("ProjectTableImportEngine from '../../import/ProjectTableImportEngine.js?v=58.00"), 'Workspace lädt die Import-Engine cache-sicher.');
...
check(html.includes('phase42_00.css?v=58.00'), 'App lädt das Phase-42-Stylesheet.');
check(html.includes('src/main.js?v=58.00'), 'App lädt Main mit Phase-42-Cache-Stand.');
check(main.includes('WorkspaceComponent.js?v=58.00'), 'Main lädt Workspace cache-sicher.');
...
check(version.includes("APP_RELEASE = '58.00'"), 'Release ist Phase 42.00.');
check(version.includes("APP_VERSION = '3.0.0'"), 'App-Version ist 1.19.0.');
check(packageJson.version === '3.0.0', 'Paketversion ist 1.19.0.');
check(packageJson.scripts['test:phase42']?.includes('run-phase42-table-import-tests.js'), 'Phase-42-Testskript ist registriert.');
check(packageJson.scripts.test.startsWith('node tests/run-phase58-final-release-tests.js'), 'Gesamttest startet mit der aktuellen Phase 56.');
check(landing.includes('Release 3.0.0 · Phase 58.00'), 'Index-Hauptseite nennt den aktuellen Stand.');
