...
const SECTION_FIELDS = [
  ...
  { key: 'roughnessMm', label: 'Rauigkeit k', unit: 'mm', digits: 2 },
  { key: 'frictionFactor', label: 'Reibungszahl λ', unit: '-', digits: 5 },
  { key: 'reynoldsNumber', label: 'Reynolds-Zahl', unit: '-', digits: 0 },
...
function normalizeSection(section = {}, result = {}, index = 0) {
  return {
    ...
    roughnessMm: round(result.roughnessMm ?? section.roughnessMm, 0.15),
    frictionFactor: round(result.frictionFactor ?? result.lambda),
    reynoldsNumber: round(result.reynoldsNumber),
  ...
  static createTechnicalSnapshot(project = {}, systemId = null, calculationInput = null) {
    ...
    return {
      ...
      settings: {
        ...
        defaultRoughnessMm: round(cloned.settings?.defaultRoughnessMm, 0.15),
        kinematicViscosity: round(cloned.settings?.kinematicViscosity, 0.0000151),
