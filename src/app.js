...
function buildPdfReport(){
  ...
  <article class="pdf-page"><header class="pdf-page-head">${pdfLogo()}<div><h2>ANLAGENINFORMATIONEN</h2></div></header><section class="pdf-two-col"><div><h3>Projektinformationen</h3><dl><dt>Projektnummer</dt><dd>${safeHtml(p.objekt)}</dd><dt>Anlage</dt><dd>${safeHtml(p.anlage)}</dd><dt>Bearbeiter</dt><dd>${safeHtml(p.bearbeiter)}</dd><dt>Datum</dt><dd>${safeHtml(p.datum)}</dd><dt>Software</dt><dd>Druckverlust Pro</dd><dt>Version</dt><dd>1.0</dd></dl></div><div><h3>Hinweise</h3><ul><li>Alle Angaben ohne Gewähr.</li><li>Für die Richtigkeit der Eingabedaten ist der Planer verantwortlich.</li><li>Diese Berechnung ersetzt keine Detailplanung.</li></ul></div></section><section class="pdf-info-box"><h3>VERWENDETE BERECHNUNGSGRUNDLAGEN</h3><ul><li>Berechnung nach den in der Software hinterlegten Formeln.</li><li>Luftdichte ρ = ${nfmt(p.rho,2)} kg/m³ | Reibungszahl λ = ${nfmt(p.lambda,3)}</li><li>Formbeiwerte ζ nach SWKI VA 103-01 / SIA 382/1</li><li>Druckverlustberechnung nach Darcy-Weisbach</li></ul></section><div class="pdf-copyright">© 2026 Emre Özgöller – Druckverlust Pro</div>${pageFooter(6)}</article>`;
}
...
async function exportProfessionalPdfWithJsPDF(){
  ...
  labelValue('Projektnummer', p.objekt, 14, 77); labelValue('Anlage', p.anlage, 14, 87); labelValue('Bearbeiter', p.bearbeiter, 14, 97); labelValue('Datum', p.datum, 14, 107); labelValue('Luftdichte ρ [kg/m³]', nfmt(p.rho,2), 14, 121); labelValue('Reibungszahl λ', nfmt(p.lambda,3), 14, 131);
  ...
  labelValue('Projektnummer', p.objekt, 14, 54); labelValue('Anlage', p.anlage, 14, 64); labelValue('Bearbeiter', p.bearbeiter, 14, 74); labelValue('Datum', p.datum, 14, 84); labelValue('Software', 'Druckverlust Pro', 14, 102); labelValue('Version', '1.0', 14, 112);
